...
class AttModel(CaptionModel):
    def __init__(self, opt):
        ...
        self.ss_prob = 0.0 # Schedule sampling probability  这个是干什么用的？解决teacher fourcing的
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        state_sample = self.init_hidden(batch_size)

        outputs = fc_feats.new_zeros(batch_size, seq.size(1) - 1, self.vocab_size+1)
        pred_outputs = fc_feats.new_zeros(batch_size, seq.size(1) - 1, self.vocab_size+1)
        logprobs_log_softmaxs = fc_feats.new_zeros(batch_size, seq.size(1) - 1, self.vocab_size+1)
        logprobs_log_softmaxs_sample = fc_feats.new_zeros(batch_size, seq.size(1) - 1, self.vocab_size+1)
        head_num = 8
        atts_distributions = fc_feats.new_zeros(batch_size, seq.size(1) - 1, head_num, att_feats.size(1))
        # print("seq", seq)
        ...
        for i in range(seq.size(1) - 1):
            # 这一步的作用是什么？
            # 是否使用teacher forceing
            # 我们需要在这里设置一个循环，从而考虑上下文的信息
            # 我们需要在这里返回以prediction作为decoder输入的t+2,t+3...的prediction
            # 我们可以直接把整句话的以prediction输入并得到整句话的prediction
            if self.training and i >= 1 and self.ss_prob > 0.0: # otherwiste no need to sample
                ...
                if sample_mask.sum() == 0:
                    ...
                    prob_prev = torch.exp(logprobs_log_softmaxs[:, i-1].detach()) # fetch prev distribution: shape Nx(M+1)
                    it.index_copy_(0, sample_ind, torch.multinomial(prob_prev, 1).view(-1).index_select(0, sample_ind))
            else:
                it = seq[:, i].clone()          
            # break if all the sequences end
            if i >= 1 and seq[:, i].sum() == 0:  # 生成了终止符
                break

            output, state, att_distribution, _, logprobs_log_softmax = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)
            outputs[:, i] = output
            logprobs_log_softmaxs[:, i] = logprobs_log_softmax
            atts_distributions[:, i] = att_distribution.squeeze()
            # last_pred_outputs[:, i] = current_pred_output
        
        # 训练几个epoch之后再用这个，不然可能就会出现nan的情况
        with torch.no_grad():
            for i in range(seq.size(1) - 1):

                ss_prob_sample = 1.0 # self.ss_prob: 1.0表示使用prediction，0.0表示ground-truth
                sample_prob = fc_feats.new(batch_size).uniform_(0, 1)
                sample_mask = sample_prob < ss_prob_sample
                sample_ind = sample_mask.nonzero().view(-1)
                it_sample = seq[:, i].data.clone()
                if i >= 1:
                    # outputs_log_softmax_sample = F.log_softmax(pred_outputs, dim=-1)
                    prob_prev_sample = torch.exp(logprobs_log_softmaxs_sample[:, i-1].detach()) # fetch prev distribution: shape Nx(M+1)
                    it_sample.index_copy_(0, sample_ind, torch.multinomial(prob_prev_sample, 1).view(-1).index_select(0, sample_ind))     
                    # print("prediction_it_sample_176_attmodel: ", it_sample)
                if i >= 1 and seq[:, i].sum() == 0:  # 生成了终止符
                    break

                output_sample, state_sample, _, att_masks_sample, logprobs_log_softmax_sample = self.get_logprobs_state(it_sample, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state_sample)
                pred_outputs[:, i] = output_sample
                logprobs_log_softmaxs_sample[:, i] = logprobs_log_softmax_sample

        return outputs, pred_outputs, atts_distributions, att_masks

    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state):
        xt = self.embed(it)

        output, state, att_distribution, att_masks = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks)
        logprobs = self.logit(output)
        logprobs_log_softmax = F.log_softmax(logprobs, dim=1)

        return logprobs, state, att_distribution, att_masks, logprobs_log_softmax
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for k in range(batch_size):
            ...
            for t in range(1):
                ...
                logprobs, state = self.get_logprobs_state(it, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_att_masks, state)[:2]
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for t in range(self.seq_length + 1):
            ...
            logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)[:2]
    ...
    def _sample_train(self, fc_feats, att_feats, seq_gt, att_masks=None, opt={}):
        ...
        seq = fc_feats.new_zeros((batch_size, seq_gt.size(1)), dtype=torch.long)
        seqLogprobs = fc_feats.new_zeros(batch_size, seq_gt.size(1))
        for t in range(seq_gt.size(1) - 1):
            # if t == 0: # input <bos>
            #     it_gt = fc_feats.new_zeros(batch_size, dtype=torch.long)
            # else:
            it_gt = seq_gt[:, t].clone()
            # break if all the sequences end
            if t >= 1 and seq_gt[:, t].sum() == 0:
                break

            logprobs, state = self.get_logprobs_state(it_gt, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state)[:2]
          
            # sample the next word
            if t == seq_gt.size(1): # skip if we achieve maximum length
                break
            it, sampleLogprobs = self.sample_next_word(logprobs, sample_method, temperature)
