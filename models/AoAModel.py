...
class AoA_Refiner_Layer(nn.Module):
    ...
    def forward(self, x, mask):
        x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, mask)[0])  # [0]保证在encoder的时候，不返回注意力分布
...
class AoA_Decoder_Core(nn.Module):
    ...
    def forward(self, xt, mean_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        # 这一步是很有意思的，对应了transformer中的mask attention modual，但是同时引入了mean_feats视觉特征
        h_att, c_att = self.att_lstm(torch.cat([xt, mean_feats + self.ctx_drop(state[0][1])], 1), (state[0][0], state[1][0]))

        if self.use_multi_head == 2:  # use_multi_head： 2
            att, att_distribution= self.attention(h_att, p_att_feats.narrow(2, 0, self.multi_head_scale * self.d_model), p_att_feats.narrow(2, self.multi_head_scale * self.d_model, self.multi_head_scale * self.d_model), att_masks)
        else:
            att = self.attention(h_att, att_feats, p_att_feats, att_masks)

        # 这一步就更有意思了
        # 为什么要将这两个拼接起来？这就和RSTNet拼接语言特征很像
        ...
        if self.decoder_type == 'LSTM':
            ...
            output = self.att2ctx(ctx_input)  # GLU操作来进行门控
        ...
        return output, state, att_distribution, att_masks
